...
1. Download the latest `remarkable-calendar-notes-<version>-armv7.zip`
   ...
   (built and mirrored by `.github/workflows/release.yml`), and verify its
   checksum against the accompanying `.sha256`/`.sha512` file.
...
installable from the official feed yet**. Its source URLs point to the
public binary distribution repository and its checksums are generated by
`scripts/update-vellum-checksums.sh`; Vellum publication additionally
requires a Vellum maintainer to review and merge a PR, then choose to
publish to their testing repository. See
